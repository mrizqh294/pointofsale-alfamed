    ...
    <x-main_content>
        ...
        <div x-data="{ isOpenDestroy: false,id: null }" x-cloak>
            ...
            <div class="flex justify-between py-3">
                <div>
                    <form action="{{ route('pemilik.getUser') }}" method="get">
                    @csrf
                    ...
                    <a href="{{ route('pemilik.getUser') }}" class="ms-1 cursor-pointer border border-teal-600 bg-gray-100 hover:bg-gray-200 text-teal-600 px-3 py-2 rounded-lg"><i class="fa-solid fa-arrows-rotate"></i></a>
                    </form>
                </div>
            </div>
            {{-- delete modal --}}
            <div class="fixed top-0 left-0 w-full h-screen bg-black/40 flex justify-center items-center" x-show="isOpenDestroy">
                <div class="p-8 bg-white w-1/3 rounded-xl">
                    ...
                    <form :action="`{{ route('pemilik.destroyUser', ':id') }}`.replace(':id', id)" method="POST">
                        @csrf
                        @method('DELETE')
