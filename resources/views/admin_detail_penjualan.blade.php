      ...
      <x-slot:title>{{ $title }}</x-slot>
        <div class="" x-data="{ isOpenAdd: false, isOpenDestroy: false, isOpenUpdate: false, id: null, current: {} }" x-cloak>
            <div class="flex justify-between py-3">
              <div>
                <a href="{{ route('penjualan.getSale') }}" class="cursor-pointer border border-teal-600 bg-gray-100 hover:bg-gray-200 text-teal-600 px-3 py-2 rounded-lg"><i class="fa-solid fa-arrow-left"></i> Kembali</a>
              </div>
            </div>
              ...
              <table class="min-w-full table-fixed border ">
                ...
                <tbody class="">
                  ...
                  <tr class="hover:bg-gray-50">
                    ...
                    <td class="text-right py-2 px-4 border border-gray-400">{{ $saleDetail->harga_jual_formatted }}</td>
                    <td class="text-center py-2 px-4 border border-gray-400">{{ $saleDetail->jumlah_obat }}</td>
                    <td class="text-right py-2 px-4 border border-gray-400">{{ $saleDetail->subtotal_penjualan_formatted }}</td>
                  </tr>
                  @endforeach
                </tbody>
              </table>
